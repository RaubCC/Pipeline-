...
// script.js (minified for copy-paste)
window.addEventListener("DOMContentLoaded",function(){const c=document.getElementById("tetris"),ctx=c.getContext("2d"),nC=document.getElementById("next"),nCtx=nC.getContext("2d"),del=document.getElementById("water-delivered"),fP=document.getElementById("fact-popup"),fT=document.getElementById("fact-text"),cF=document.getElementById("close-fact"),pB=document.getElementById("pause-btn"),sb=document.getElementById("sidebar"),lvlD=document.getElementById("level-display"),oR=document.getElementById("overlay-restart"),fR=document.getElementById("fact-rotator"),bL=document.getElementById("btn-left"),bR=document.getElementById("btn-right"),bRt=document.getElementById("btn-rotate"),bD=document.getElementById("btn-drop"),l2n=document.getElementById("lines-to-next"),hM=document.getElementById("how-to-modal"),cH=document.getElementById("close-how-to"),sT=document.getElementById("sidebar-toggle"),wW=document.getElementById("can-water"),wL=document.getElementById("can-liters"),ROWS=20,COLS=10,BLOCK=30;let board,cur,nextP,pos,lit,ds,go,lc,level,ln2lvl,mChance,speeds,paused,fi=0;const SHAPES=[[[1,1,1,1]],[[2,2],[2,2]],[[0,3,0],[3,3,3]],[[0,4,4],[4,4,0]],[[5,5,0],[0,5,5]],[[6,0,0],[6,6,6]],[[0,0,7],[7,7,7]],[[8]]],COLORS=[null,"#FFC907","#2E9DF7","#8BD1CB","#4FCB53","#F5402C","#FF902A","#F16061","#FFD700"],NAMES=["","I-pipe","O-pipe","T-pipe","S-pipe","Z-pipe","J-pipe","L-pipe","Jerry Can"],FACTS=["Every $1 invested in clean water yields $4–$12 in economic returns.","Women and girls spend 200 million hours every day collecting water.","1 in 10 people on Earth don’t have access to clean water.","Access to clean water reduces global disease and increases education.","Over 2 billion people use a drinking water source contaminated with feces.","Clean water saves lives. You’re helping, line by line.","Diarrhea from unsafe water kills more children than malaria, measles, and HIV/AIDS combined.","Clean water = more time for school, work, and dreams.","You’re not just clearing lines. You’re delivering hope."],MUD=9;SHAPES.push([[MUD]]);COLORS.push("#8B5C2A");NAMES.push("Mud Block");c.width=COLS*BLOCK;c.height=ROWS*BLOCK;function rnd(){let r=Math.random(),i;if(r<0.03)i=7;else if(r<0.03+mChance)i=8;else i=Math.floor(Math.random()*7);return{shape:SHAPES[i],color:COLORS[i+1],idx:i+1,name:NAMES[i+1]}}function db(x,y,clr,ct=ctx){ct.fillStyle=clr;ct.fillRect(x*BLOCK,y*BLOCK,BLOCK,BLOCK);ct.strokeStyle="#222";ct.lineWidth=2;ct.strokeRect(x*BLOCK,y*BLOCK,BLOCK,BLOCK);if(clr===COLORS[8]){ct.fillStyle="#FFC907";ct.fillRect(x*BLOCK+BLOCK*0.2,y*BLOCK+BLOCK*0.4,BLOCK*0.6,BLOCK*0.4);ct.fillRect(x*BLOCK+BLOCK*0.35,y*BLOCK+BLOCK*0.2,BLOCK*0.3,BLOCK*0.25)}if(clr===COLORS[9]){ct.fillStyle="#6B3E14";ct.beginPath();ct.arc(x*BLOCK+BLOCK/2,y*BLOCK+BLOCK/2,BLOCK/4,0,2*Math.PI);ct.fill()}}function draw(){ctx.clearRect(0,0,c.width,c.height);for(let y=0;y<ROWS;y++)for(let x=0;x<COLS;x++)board[y][x]&&db(x,y,COLORS[board[y][x]]);cur.shape.forEach((row,yy)=>row.forEach((v,xx)=>v&&db(pos.x+xx,pos.y+yy,cur.color)))}function dNext(){nCtx.clearRect(0,0,nC.width,nC.height);nextP.shape.forEach((row,yy)=>row.forEach((v,xx)=>v&&db(xx+1,yy+1,nextP.color,nCtx)))}function valid(dx=0,dy=0,m=cur.shape){for(let y=0;y<m.length;y++)for(let x=0;x<m[y].length;x++)if(m[y][x]){let nx=pos.x+x+dx,ny=pos.y+y+dy;if(nx<0||nx>=COLS||ny>=ROWS)return!1;if(ny>=0&&board[ny][nx])return!1}return!0}function showFact(f){if(f){fT.textContent=f;fP.style.display="block";paused=!0;pB.textContent="Resume"}else{fR.textContent=FACTS[fi];fi=(fi+1)%FACTS.length}}function merge(){if(cur.idx===8){for(let x=0;x<COLS;x++)board[pos.y][x]=0;lit+=400;showFact("JERRY CAN POWER! Doubled liters for this row!")}else if(cur.idx===9){board[pos.y][pos.x]=9;lit=Math.max(0,lit-100);del.textContent=`Liters Delivered: ${lit}`;showFact("Oh no! Mud block! -100 liters delivered.")}else cur.shape.forEach((row,yy)=>row.forEach((v,xx)=>v&&(board[pos.y+yy][pos.x+xx]=cur.idx)))}function rot(m){return m[0].map((_,i)=>m.map(r=>r[i])).reverse()}function clrLn(){let cnt=0;board=board.filter(r=>r.every(v=>v)?(cnt++,!1):!0);while(board.length<ROWS)board.unshift(Array(COLS).fill(0));return cnt}function ani(o,n){let step=Math.max(1,Math.floor((n-o)/16)),v=o,up=n>o;del.classList.remove("score-up","score-down");del.classList.add(up?"score-up":"score-down");(function t(){if((up&&v<n)||(!up&&v>n)){v+=up?step:-step;if((up&&v>n)||(!up&&v<n))v=n;del.textContent=`Liters Delivered: ${v}`;requestAnimationFrame(t)}else{del.textContent=`Liters Delivered: ${n}`;setTimeout(()=>del.classList.remove("score-up","score-down"),400)}})()}const splash=document.getElementById("splash-effect");function pop(){let s=document.createElement("div");s.className="splash";let ox=(Math.random()-0.5)*120,oy=(Math.random()-0.5)*60;s.style.left=`calc(50%+${ox}px)`;s.style.top=`calc(40%+${oy}px)`;splash.appendChild(s);setTimeout(()=>s.remove(),700)}function conf(){let cols=["#FFC907","#2E9DF7","#8BD1CB","#4FCB53","#F5402C","#FF902A","#F16061"];for(let i=0;i<32;i++){let d=document.createElement("div");d.style.position="absolute";d.style.left=`${50+(Math.random()-0.5)*40}%`;d.style.top="40%";d.style.width="12px";d.style.height="12px";d.style.background=cols[Math.floor(Math.random()*cols.length)];d.style.borderRadius="3px";d.style.opacity="0.85";d.style.transform=`rotate(${Math.random()*360}deg)`;d.style.zIndex="301";d.style.transition="all 1.1s cubic-bezier(.4,1.4,.6,1)";setTimeout(()=>{d.style.top=`${60+Math.random()*30}%`;d.style.left=`${50+(Math.random()-0.5)*60}%`;d.style.opacity="0"},10);setTimeout(()=>d.remove(),1200);splash.appendChild(d)}}function updCan(w){let m=2000,pct=Math.min(w/m,1);wW.style.height=`${Math.floor(pct*120)}px`;wL.textContent=`${w}L`;if(pct===1){wW.style.boxShadow="0 0 24px 8px #2E9DF7aa";setTimeout(()=>wW.style.boxShadow="",600)}}function updLit(lines){if(lines){pop();let o=lit;lit+=lines*200;lc+=lines;ani(o,lit);updCan(lit);showFact();updLvl();updL2N()}}function updLvl(){if(lc>=ln2lvl&&level<5){level++;lc=0;ln2lvl+=8;[null,600,450,350,250,150]/*dummy*/;if(level===2){mChance=0.18;speeds[1]=450}if(level===3){mChance=0.28;speeds[2]=350}if(level===4){mChance=0.40;speeds[3]=250}if(level===5){mChance=0.55;speeds[4]=150}showFact(`Level Up! Welcome to Level ${level}.`);conf();lvlD.textContent=`Level: ${level}`}}function updL2N(){let x=Math.max(0,ln2lvl-lc);l2n.textContent=`Next Level in ${x} line${x===1?"":"s"}`}function rotFly(){fR.textContent=FACTS[fi];fi=(fi+1)%FACTS.length}function td(){if(go||paused)return;let now=Date.now(),d=now-ds,sp=speeds[level-1]||200;if(d>sp){if(valid(0,1))pos.y++;else{merge();let L=clrLn();updLit(L);reset();if(!valid())return go=!0,oR.classList.add("active")}ds=Date.now()}draw();if(!go&&!paused)requestAnimationFrame(td)}function reset(){cur=nextP;nextP=rnd();pos={x:3,y:0};dNext()}function k(e){if(go&&e.key!=="Enter")return;e.key==="ArrowLeft"&&valid(-1,0)&&pos.x--;e.key==="ArrowRight"&&valid(1,0)&&pos.x++;e.key==="ArrowDown"&&valid(0,1)&&pos.y++;if(e.key==="ArrowUp"){let r=rot(cur.shape);valid(0,0,r)&&(cur.shape=r)}if(e.key===" "){while(valid(0,1))pos.y++}if(e.key==="Enter"&&go){start();fP.style.display="none"}draw()}document.addEventListener("keydown",e=>{["ArrowLeft","ArrowRight","ArrowUp","ArrowDown"," ","Spacebar"].includes(e.key)&&e.preventDefault();k(e)});pB.onclick=()=>{paused=!paused;pB.textContent=paused?"Resume":"Pause";paused||td()};sT.onclick=()=>sb.classList.toggle("collapsed");document.getElementById("instructions").onclick=()=>hM.style.display="flex";cH.onclick=()=>hM.style.display="none";cF.addEventListener("click",()=>{fP.style.display="none";paused=false;pB.textContent="Pause";td()});function start(){lit=0;lc=0;level=1;ln2lvl=8;mChance=0.1;ds=[600,450,350,250,150];board=Array.from({length:ROWS},()=>Array(COLS).fill(0));cur=rnd();nextP=rnd();pos={x:3,y:0};go=false;paused=false;pB.textContent="Pause";sb.classList.remove("collapsed");dNext();del.textContent="Liters Delivered: 0";updCan(0);lvlD.textContent="Level: 1";updL2N();ds=Date.now();td()}document.getElementById("restart").onclick=start;oR.onclick=()=>{oR.classList.remove("active");start()};oR.addEventListener("keydown",e=>{(e.key==="Enter"||e.key===" ")&&(oR.classList.remove("active"),start())});setInterval(rotFly,8000);rotFly();if(bL&&bR&&bRt&&bD){const bind=(el,fn)=>{el.addEventListener("touchstart",fn);el.addEventListener("click",fn)};bind(bL,e=>{e.preventDefault();valid(-1,0)&&(pos.x--,draw(),ds=Date.now())});bind(bR,e=>{e.preventDefault();valid(1,0)&&(pos.x++,draw(),ds=Date.now())});bind(bRt,e=>{e.preventDefault();let r=rot(cur.shape);valid(0,0,r)&&(cur.shape=r,draw(),ds=Date.now())});bind(bD,e=>{e.preventDefault();while(valid(0,1))pos.y++;draw(),ds=Date.now()})}const oO=document.getElementById("game-over-overlay");function hideO(){oO&&(oO.style.display="none",oO.classList.remove("active"))}start();});
